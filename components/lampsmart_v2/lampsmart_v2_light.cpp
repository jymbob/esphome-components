...
namespace esphome
{
  namespace lampsmartv2
  {
    static const char *TAG = "lampsmartv2";
    ...
    char *LampSmartV2Light::getHostDeviceIdentifier()
    {
    ...
    void LampSmartV2Light::write_state(light::LightState *state)
    {
      float cwf, wwf;
      ...
      ESP_LOGD(TAG, "LampSmartV2Light::write_state called! Requested cw: %d, ww: %d", cwi, wwi);
    ...
    void LampSmartV2Light::dump_config()
    {
      ESP_LOGCONFIG(TAG, "LampSmartV2Light '%s'", light_state_ ? light_state_->get_name().c_str() : "");
    ...
    void LampSmartV2Light::on_pair()
    {
      ESP_LOGD(TAG, "LampSmartV2Light::on_pair called!");

      char *hostId = getHostDeviceIdentifier();
      send_packet(CMD_PAIR, hostId[0], hostId[1]);
    }

    void LampSmartV2Light::on_unpair()
    {
      ESP_LOGD(TAG, "LampSmartV2Light::on_unpair called!");

      char *hostId = getHostDeviceIdentifier();
      send_packet(CMD_UNPAIR, hostId[0], hostId[1]);
    }

    void LampSmartV2Light::send_packet(uint16_t cmd, uint8_t arg1, uint8_t arg2)
    {
      char *hostId = getHostDeviceIdentifier();
